...
def generate_health_plan(user_profile, provider, ollama_url=None, ollama_model=None):
...
"""
    ...
    if provider == "OpenAI":
        ...
        try:
            with st.spinner(f"🤖 AI (Ollama - {ollama_model}) is creating your personalized health plan... This may take a moment..."):
                full_prompt = system_instruction + "\n\n" + prompt
                response = requests.post(f"{ollama_url.rstrip('/')}/api/generate", json={
                    "model": ollama_model,
                    "prompt": full_prompt,
                    "stream": False
                })
...
def main():
    ...
    with st.sidebar:
        ...
        provider = st.selectbox("Select AI Provider", ["OpenAI", "Gemini", "Ollama"], index=0)
        ...
        if provider == "Ollama":
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if models:
                        ollama_model = st.selectbox("Select Model", models)
                    else:
                        st.warning("No models found. Please pull a model first (e.g., `ollama run llama3`).")
                else:
                    st.error("Failed to fetch models from Ollama.")
            except Exception:
    ...
    if submitted:
        ...
        if provider == "Ollama" and not ollama_model:
            st.error("❌ Please select an Ollama model before generating the plan.")
            return

        plan = generate_health_plan(user_profile, provider, ollama_url, ollama_model)
        ...
        st.markdown("""
        <div class="info-box">
        <strong>Meet Mr. Dhar:</strong> Recently diagnosed with Type 1 Diabetes, aiming for weight loss.<br><br>
